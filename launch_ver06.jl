...
Nsamples=10000
paper ? srand(111) : nothing
PMI, PMIstd, PMIrate, PMIratestd = PathMI.estimate(S,times,Nsamples)
paper ? figure("figure1b",figsize=(4.1,4)) : figure("figure1b")
...
if paper
    figure("figure1b")
    subplots_adjust(top=0.995,bottom=0.115,left=0.15,right=0.995,hspace=0.2,wspace=0.2)
    #figure("MIrate1")
...
times=collect(0:1.:200)
Nsamples=10000
paper ? srand(1234567) : nothing
PMI,PMIstd, PMIrate, PMIratestd=PathMI.estimate(S,times,Nsamples)
paper ? figure("figure1d",figsize=(4.1,4)) : figure("figure1d")
plot(times,PMI,color="xkcd:royal blue")
xlabel("time",fontdict=font1)
ylabel("path mutual information [nats]",fontdict=font1)
...
#plot(times,PMIrate,color="xkcd:royal blue")
#xlabel("time",fontdict=font1)
#ylabel(L"\frac{\mathbb{M}(\mathcal{A}_t,\mathcal{B}_t)}{t}",size=16,rotation=0.,position=(-1.,1.))
#ylabel("path mutual information rate",fontdict=font1)
for i=1:20
    I,Xt=PathMI.SSA(S,times)
    A0=find(Xt.x[1,:].==0)[1]
    join=find(times.>Xt.t[A0])
    tt=Xt.t[1:A0]
    Info=Xt.pmi[1:A0]
...
Nsamples=1000
paper ? srand(1234) : nothing
PMI,PMIstd, PMIrate, PMIratestd=PathMI.estimate(S,times,Nsamples)
figure("figure2b",figsize=(7.2,3.7))
plot(times,PMI,color="xkcd:royal blue")
xlabel("time",fontdict=font1)
ylabel("path mutual information [nats]",fontdict=font1)
#figure("MIrate3",figsize=(7.2,3.7))
#plot(times,PMIrate,color="xkcd:royal blue")
#xlabel("time",fontdict=font1)
#ylabel("Path mutual information rate",fontdict=font1)
for i=1:20
    ...
    figure("figure2b")
    plot(tt,Info,color="xkcd:royal blue",alpha=0.2,lw=0.5)
    paper ? subplots_adjust(top=0.965,bottom=0.125,left=0.11,right=0.91) : nothing
    #figure("MIrate3")
    #plot(tt,[0.;Info[2:end]./tt[2:end]],color="xkcd:royal blue",alpha=0.1,lw=0.5)
...
function figure2a(;paper=false)
    ...
    figure("figure2a",figsize=(7.2,3.7))
    cc=["orange","green"]
    paper ? srand(790966) : nothing
    ...
    ylabel("path-MI sample [nats]",size=14,color="xkcd:royal blue")
    paper ? subplots_adjust(top=0.965,bottom=0.125,left=0.11,right=0.91) : nothing
return tt,Info
end
